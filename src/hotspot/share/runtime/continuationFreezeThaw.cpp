      ...
      if (omv.type() == OopMapValue::callee_saved_value) {
        ...
        if (loc != nullptr) {
          if (*loc == (intptr_t)_thread) {
            // This register location contains the current carrier thread pointer,
            // which is correct - it was already updated by handle_preempted_continuation
            log_develop_debug(continuations)("recurse_thaw_stub_frame post fix_thawed_frame: register %s at " INTPTR_FORMAT " already has new thread " INTPTR_FORMAT,
                                            reg->name(), p2i(loc), p2i(_thread));
          } else if (*loc == (intptr_t)old_thread) {
            // This register location contains the old carrier thread pointer,
            // so we need to update it to the new thread pointer.
            log_develop_debug(continuations)("recurse_thaw_stub_frame post fix_thawed_frame: updating register %s at " INTPTR_FORMAT " from old thread " INTPTR_FORMAT " to new thread " INTPTR_FORMAT,
                                            reg->name(), p2i(loc), p2i(old_thread), p2i(_thread));
            *loc = (intptr_t)_thread;
            updated++;
          } else {
            log_develop_debug(continuations)("recurse_thaw_stub_frame post fix_thawed_frame: no update required for register %s at " INTPTR_FORMAT,
                                            reg->name(), p2i(loc));
          }
